...
function setActiveStyleSheet(title) {
    ...
    if (color_skin == 'dark') {
        ...
        jQuery("a.logo img").attr('src', 'assets/resources/images/logo_dark.png');
    }
}
...
document.addEventListener("DOMContentLoaded", function(event) {
    // lightVersion initionlize
    let params = (new URL(document.location)).searchParams;
    if (params.has("version")) {
        if (params.get("version") === 'light') {
            createCookie("is_dark", 'false', 365);
        }
        if (params.get("version") === 'dark') {
